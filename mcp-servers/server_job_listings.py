...
def get_job_ids(job_title: str, city:str, radius:int) -> str:
    ...
    response = requests.get('https://rest.arbeitsagentur.de/jobboerse/jobsuche-service/pc/v4/jobs',
                            headers=HEADERS, params=params, verify=True).json()
    jobs_str = [{"job_title":job_dict["titel"], "employee_name":job_dict["arbeitgeber"], "refnr":job_dict["refnr"]} for job_dict in response["stellenangebote"]]
